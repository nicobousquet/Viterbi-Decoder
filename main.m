...
N = 2048; % Nombre de bits codés par trame (codée)
...
nbrErreur  = 20;  % Nombre d'erreurs à observer avant de calculer un BER
...
for iSNR = 1:length(EbN0dB)
    ...
    while (bitErr < nbrErreur && pqtNbr*K < nbrBitMax)
        ...
        %u_rec = vitdec(y, trellis,1,'trunc','unquant');
    ...
    d0 = 1;
    d1 = 100;
    %TEP_bis(iSNR) = impulsion(K, N, d0, d1, trellis, s_i, closed, iSNR); 
    refreshdata(hTEB);
    refreshdata(hTEP);
    drawnow limitrate
